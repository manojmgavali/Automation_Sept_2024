...
public class Cartfunctionality_Steps {
	...
	public Cartfunctionality_Steps() {
		ExtentSparkReporter spark = new ExtentSparkReporter("target/cucumber-reports/CartFunctionality.html");
	...
	public void user_click_on_the_button(String buttonText) throws InterruptedException {
		...
		try {
			...
			button.click();
			test.log(Status.PASS, "Successfully clicked on button: " + buttonText);
		} catch (NoSuchElementException e) {
			test.log(Status.FAIL, "Button with text '" + buttonText + "' not found.");
			Assert.fail("Button with text '" + buttonText + "' not found: " + e.getMessage());
		} catch (Exception e) {
			test.log(Status.FAIL, "An unexpected error occurred while clicking the button: " + buttonText);
...
//        test.log(Status.PASS, "Clicked on button: " + buttonText);
	...
	public void enter_checkout_information(io.cucumber.datatable.DataTable dataTable) {
		...
		String name = details.get(0).get(1);
		String address = details.get(1).get(1);
		String postalCode = details.get(2).get(1);
	...
	public void the_order_confirmation_page_should_be_displayed() {
		...
		WebElement confirmationMessage = driver
				.findElement(By.xpath("//h2[normalize-space()='Thank you for your order!']"));
